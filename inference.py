...
def driven_by_flame_coefs(model, src_pth, out_dir, device):
    ...
    src_img, src_crop, src_tform = preprocess_image(src_pth, fa, device)
    ...
    with torch.no_grad():
        ...
        for i in range(10):
            ...
            shape[0, 0] = 3.0 * i / 9  # wider range, full sweep
            outputs = model.flame_coef_generation(
                src_crop, src_img, src_tform,
                shape=shape, pose=pose
            )
    ...
    with torch.no_grad():
        ...
        for i in range(10):
            ...
            exp[0, 1] = 3.0 * i / 9  # ← use index 1, not 0
            outputs = model.flame_coef_generation(
                src_crop, src_img, src_tform,
                exp=exp, pose=pose
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src_pth", default="examples/1.png")
    parser.add_argument("--ckpt_pth", default="checkpoints/ckpt_45.pt")
